...
// Optional OTP + Email services (used only if explicitly enabled)
  ...
  try {
    ...
    passport.authenticate('google', { callbackURL }, (err, user, info) => {
      ...
      try {
        ...
        // Decide whether OTP is required after OAuth (default: false)
        const requireOtp = String(process.env.OAUTH_REQUIRE_OTP || 'false').toLowerCase() === 'true';
        ...
        if (!requireOtp) {
          ...
          try {
            ...
            // Set httpOnly cookie for same-origin sessions
            try {
            ...
            const url = `${postLoginPath}${sep}token=${encodeURIComponent(token)}`;
            return finalOrigin ? res.redirect(`${finalOrigin}${url}`) : res.redirect(url);
          } catch (tokenErr) {
            console.error('❌ Token generation error after OAuth:', tokenErr);
            return res.redirect(`${mountPath}/failure?error=token_generation_failed`);
          }
        }

        // OTP-enabled path (legacy / stricter mode)
        (async () => {
          try {
            ...
            const finalOrigin = returnOrigin || frontOrigin;
            const verifyPath = `/verify-otp?email=${encodeURIComponent(user.email)}&flow=login&provider=google&redirect=${encodeURIComponent(postLoginPath)}`;
            return finalOrigin ? res.redirect(`${finalOrigin}${verifyPath}`) : res.redirect(verifyPath);
          } catch (otpErr) {
            console.error('❌ Failed to send login OTP after Google OAuth:', otpErr);
            // Graceful fallback: log user in even if email provider blocks SMTP
            try {
              const token = generateToken(user);
              try {
                res.cookie('authToken', token, { httpOnly: true, sameSite: 'Lax', secure: true, maxAge: 7 * 24 * 60 * 60 * 1000 });
              } catch (_) {}
              const finalOrigin = returnOrigin || frontOrigin;
              const sep = postLoginPath.includes('?') ? '&' : '?';
              const url = `${postLoginPath}${sep}token=${encodeURIComponent(token)}&otp=skipped`;
              return finalOrigin ? res.redirect(`${finalOrigin}${url}`) : res.redirect(url);
            } catch (_) {
              return res.redirect(`${mountPath}/failure?error=otp_send_failed`);
            }
          }
        })();
      } catch (routeErr) {
        console.error('❌ Google OAuth route processing error:', routeErr);
        console.error('Token/route error stack:', routeErr.stack);
        res.redirect(`${mountPath}/failure?error=route_processing_error`);
      }
    })(req, res, next);
  } catch (routeError) {
...
// OAuth success page (kept for completeness)
